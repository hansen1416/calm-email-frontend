<template>
  ...
  <div class="send-card">
    ...
    <div class="send-section">
      ...
      <div class="send-section">
        <label>{{ $t('sendEmail.selectSegments') }}</label>
        <el-select v-model="form.segment_ids" multiple :placeholder="$t('sendEmail.selectSegments') + ' (multiple)'" style="width:100%" size="large">
...
async function loadData() {
  // M7: 同时加载发件人列表
  const [t, c, g, seg, s] = await Promise.all([
...
async function handleSend() {
  if (!form.value.template_id) { ElMessage.warning(t('sendEmail.selectTemplateMsg')); return }
    if (!form.value.contact_ids.length && !form.value.group_ids.length && !form.value.segment_ids.length) {
